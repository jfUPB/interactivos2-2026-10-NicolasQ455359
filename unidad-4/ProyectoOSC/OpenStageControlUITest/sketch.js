...
function handleOSCMessage(msg) {
  ...
  // reconstruir address si no viene
  if (!msg.address && msg.args && msg.args.length) {
    msg.address = "/" + String(msg.args[0]);
  }
  ...
  if (msg.address === "/drone") {
    fogValue = min(1, fogValue + 0.05);
    return;
  }
  ...
  if (msg.address === "/melody") {
    spawnPickups(1);
    sceneFlash = 0.5;
    return;
  }

  if (msg.address === "/events") {
    shake = max(shake, 10);
    sceneFlash = 1;
    spawnEnemies(2);
    return;
...
function connectWS() {
  ...
  ws.onopen = () => {
    wsOk = true;
    lastPreview = "WS Strudel conectado ✅";
  };
  ws.onclose = () => {
    wsOk = false;
    lastPreview = "WS Strudel cerrado ⚠️";
  };
  ws.onerror = () => {
    wsOk = false;
    lastPreview = "WS Strudel error ⚠️";
  };
...
function connectOSC() {
  ...
  wsOSC.onclose = () => {
    wsOSCOk = false;
    console.log("WS OSC cerrado ⚠️ — reintentando en 3s");
...
function renderHUD() {
  ...
  text(`Strudel: ${wsOk ? "✅ (8081)" : "⚠️ off"}  |  OSC: ${wsOSCOk ? "✅ (8082)" : "⚠️ off"}`, 28, 24);
  ...
  fill(255, 255, 255, 180);
  text(`Audio: controlado por Strudel`, 28, 178);
