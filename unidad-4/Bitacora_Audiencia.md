...
```mermaid
graph TD;
    subgraph Generacion de Audio
        MA[Motor Audio: Web Audio API] --> AO[Audio Output]
        AO --> E[Escucha de Publico e Interpretes]
    end

    subgraph Live Coding Musical
        S[Strudel] --> MA
        S -- Broadcast OSC --> EH[Eventos Generativos]
        EH --> B1[Bridge Strudel: Node WS p.8081]
    end

    subgraph Control Manual Directo
        OSC[Open Stage Control] -- Knob /biome --> UDP[UDP Local p.9000]
        UDP --> B2[bridgeUI.js: Node WS p.8082]
    end

    subgraph Participacion del Publico
        CEL[Dispositivos Moviles Red Local] -- Interfaz Web HTTP:3000 --> SERVER[audienceBridge: Node server.js]
        SERVER <-->|Estado Colectivo / Votos WS p.8083| B3[Router Audiencia]
    end

    subgraph Motor Visual p5.js
        B1 -.->|ws://localhost:8081| P5[p5.js visualesHouse.html]
        B2 -.->|ws://localhost:8082| P5
        B3 -.->|ws://localhost:8083| P5
        P5 --> HUD[HUD y Parametros de Estado Global]
        HUD --> RENDER[Generacion de Biomas, Particulas, Enemigos]
    end
```
